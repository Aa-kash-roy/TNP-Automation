...
async function login({
  ...
  if (!user || user.validated == false) return "User doesn't exist !!"; // synchronously compare user entered password with hashed password

  if (user && bcrypt.compareSync(password, user.password)) {
    const token = auth.generateAccessToken(email);
    console.log(token); // call toJSON method applied during model instantiation
...
async function authenticateUser(params) {
  ...
  if (UserOtpRecords.length <= 0) throw new Error("Account result don't exist !!");else {
    const {
      expiredAt
    } = UserOtpRecords[0];
    ...
    if (expiredAt < Date.now()) {
      ...
      if (otp == params.otp) await User.updateOne({
        email: params.email
      }, {
        validated: true
      }).then();
...
async function getById(id) {
  const user = await User.findById(id); // call toJSON method applied during model instantiation
