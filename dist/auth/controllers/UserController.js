...
router.post('/register/otpvalidation', (req, res, next) => {
  userServices.authenticateUser(req.body).then(res.redirect('/users/login')).catch(err => next(err));
});
...
router.post('/login/setpassword', (req, res) => {
  userServices.setNewPassword(req.body).then(res.send("Password Updated !!"));
});
...
router.get('/:id', (req, res, next) => {
  userServices.getById(req.params.id).then(user => res.json(user)).catch(err => next(err));
});
